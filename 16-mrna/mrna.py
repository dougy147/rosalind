...
def count_potential_rna_strings_sources(protein):
    count = len([stop for stop in codon_table if codon_table[stop] == "Stop"])
    for c in reversed(protein):
        count *= len([x for x in codon_table if codon_table[x] == c]) % 1_000_000
    return count % 1_000_000

print(count_potential_rna_strings_sources(inp))
